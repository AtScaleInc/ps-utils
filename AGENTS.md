...
**Any non-operation command or menu item must be added to `src/scripts/generate-extension-contributes.ts`, never to `vscode-extension/package.json` directly** — the next `npm run build` overwrites `commands`/`submenus`/`menus` wholesale. The `psUtils.settings` command and the `SML_MENU` / `MONITOR_MENU` entries in that script are the examples to follow. A command declared there must also be registered in the extension, or the menu item renders and silently does nothing; the "registers every SML command it contributes" test in `src/scripts/__tests__/generate-sml-schema.test.ts` guards this, and its list of source files has to grow when a new module starts owning `psUtils.sml.*` commands. `SML_MENU` uses `when` clauses over two runtime context keys (`psUtils.yamlExtensionInstalled`, `psUtils.smlEnabled`, both set by `setSmlContextKeys` in `vscode-extension/src/sml-schema.ts`) so that three commands share one menu slot and the visible label reflects current state — the technique to reuse for any state-dependent menu label, since a menu item's `title` is static in package.json.
...
- **Schemas stay permissive** (`additionalProperties: true`). The specification lags the implementation — `sml-serializer.ts` emits `visualize_in_bi_tool`, which no reference doc mentions — so a closed schema would flag valid files, including ps-utils' own output. `src/scripts/__tests__/fixtures/sml-corpus/` is the regression test for this.
  ...
  Cross-file and engine-level checking is a different channel again: `vscode-extension/src/sml-monitor.ts` spawns the `atscale-list-model-errors` CLI operation on a file watcher and maps its JSON report onto source positions (`sml-monitor-core.ts`, pure and tested from the repo root). The operation reports problems against SML object names rather than file positions, so that mapping is best-effort by nature — it must return "unlocated" rather than guess, and every problem has to reach the output channel whether or not it mapped. If the operation's `location` string formats or its stdout contract change, `sml-monitor-core.ts` is what breaks.

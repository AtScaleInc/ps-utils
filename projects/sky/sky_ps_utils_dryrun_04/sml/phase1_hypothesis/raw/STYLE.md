...
| **PII Exclusion** | MEDIUM and above — columns at lower severity are included |
| **Label Style** | none — raw source table/column name used as label, no transformation |
| **File Naming** | snake_case — source table name used as-is (e.g. `fact_inventory_transaction.yml`) |
| **Metric Labels** | none — raw source column + underscore + aggregation (e.g. `primary_quantity_sum`) |
| **Fact Tables** | Auto-detected via FK topology and naming patterns |
| **Sample Size** | Disabled |
| **Hierarchies per Dim** | min 1 / max 4 per dimension |
...
| Setting | Value |
|---|---|
| **min-hierarchies-per-dim** | 1 — dimensions with fewer hierarchies are excluded from the model |
| **max-hierarchies-per-dim** | 4 — if more hierarchies are inferred, only the first 4 are kept |
...
PII exclusion threshold: **MEDIUM** — columns at this severity
...
| Severity | Examples |
|---|---|
| HIGH | SSN / national ID, passwords, credit card / payment card numbers, biometric identifiers |
| MEDIUM | Email addresses, phone numbers, date of birth, government ID numbers |
...
The catalog label for this generation is **SkyDryRun04Phase1Hypothesis**.

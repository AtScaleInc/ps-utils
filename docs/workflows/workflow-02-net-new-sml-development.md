...
[`generate-queries-from-sml`](../../README.md#generate-queries-from-sml) produces a total query for each metric in `metrics/` and level-breakdown queries across the model — it does not read `calculations/`, so calculated metrics need their own test coverage, not just their own expected results. Generated queries verify useful execution coverage only after they run; they do not prove business correctness without an approved expected result or comparison basis.

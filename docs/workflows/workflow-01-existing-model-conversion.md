...
| --- | --- | --- |
| Deferred or skipped calculations | Tabular conversion writes `DEFERRED_MEASURES.md` and `CONVERSION_REPORT.md`/`.json`; the AtScale XML converter reports converter-specific omissions/notes. The Multidimensional converter does not read MDX calculation scripts at all, so it produces no report on skipped calculated members — their existence must be confirmed directly against the source cube. | Inspect every source calculation and record one disposition; do not assume one converter's report covers another format, and do not assume the Multidimensional converter surfaces calculated measures it drops. |
